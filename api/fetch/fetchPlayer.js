    ...
    await fifa
        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                const nationalityImageUrl = $("a.link-nation img.nation").attr(
                    "src"
                );
                ...
                const futgg = $("a.btn.btn-block.btn-sm.btn-primary.mb-3").attr(
                    "href"
                );
                ...
                $('p:contains("Preferred Positions") a.link-position').each(
                    (i, element) => {
                        prefered_positions.push({
                            id: parseInt($(element).attr("href").split("=")[1]),
                ...
                const teams = [];
                $("div.col-12.col-sm-6.col-lg-6.team").each((i, element) => {
                    ...
                    const positionId = parseInt(
                        $(element)
                            .find('p:contains("Position") a.link-position')
                            .attr("href")
                            .split("=")[1]
                    );
                    ...
                    const kitNumber = parseInt(
                        $(element)
                            .find('p:contains("Kit Number") span.float-right')
                            .text()
                    );
                    const joinedClub = $(element)
                        .find('p:contains("Joined Club") span.float-right')
                        .text();
                    const contractLength = $(element)
                        .find('p:contains("Contract Length") span.float-right')
                        .text();
                ...
                const traits = [];
                $('div.card.mb-5:contains("Traits") p').each((i, element) => {
